...
Correct menu shows: **Installer · Manager** and version `2026.09.15t`  
...
- `NOWPAYMENTS_PAY_CURRENCY` — preferred pay coin on hosted checkout (default `usdttrc20`)
...
Invoices are priced in **USD**. Default pay coin is **USDT TRC20** (Tron). On this merchant account, NOWPayments rejects payments under ~**$15** (verified live for BTC and USDT TRC20); the donate form minimum matches that floor. Other merchants can have lower mins — that is account-side, not something the donate panel can override.
