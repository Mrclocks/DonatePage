...
Correct menu shows: **Installer · Manager** and version `2026.09.15p`  
...
- `NOWPAYMENTS_PAY_CURRENCY` — soft-preferred donor pay coin on hosted checkout (default `usdtbsc`; skipped automatically when the amount is below that coin's network minimum so $5 donations still work)
...
Invoices are priced in **USD** (`price_currency=usd`) so hosted Confirm works on typical merchant accounts that reject USDT as a *price* currency. Donors still pay crypto; USDT BEP20 is soft-preferred only when the amount clears NOWPayments' network minimum (otherwise the gateway opens for any enabled coin).
